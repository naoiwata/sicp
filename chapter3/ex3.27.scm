...
; memo-fib手続きを(memorize fib)にすると, この手続きのfibは毎回fib手続きを呼び出し評価する為, 手続きのステップ数はθ(Φ^n) (Φ: 黄金比)となり指数的に増加する. 
; 一方memo-fibを使う場合は, 一度評価されたfib手続きはtableに保存される(キャッシュとして残る)ので, 一度評価したfib手続きかどうかlookup手続きによって値を探索できるため, ステップ数はθ(n)となり線形的に増加する. 
